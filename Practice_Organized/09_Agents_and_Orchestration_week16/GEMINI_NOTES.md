...
## 8. Native function calling (stretch goal for Lab 3 — Agents)
This lab's `agent.py` uses a portable, prompt-based JSON action format instead of any
provider's native function-calling API, so the same code works across Gemini, OpenRouter,
and mock mode. If you want to try Gemini's actual native tool-calling support as a stretch
goal: pass a list of `types.FunctionDeclaration` objects (name, description, parameter
schema) via `config=types.GenerateContentConfig(tools=[...])`, and Gemini will return a
`function_call` part instead of plain text when it wants to invoke a tool — your code
still executes the real function and feeds the result back, but the request/response
shape is more structured than the JSON-in-text approach used here. Worth trying once
you've got the portable version working, since native function calling is generally more
reliable at producing well-formed tool calls than asking a model to emit raw JSON in text.
