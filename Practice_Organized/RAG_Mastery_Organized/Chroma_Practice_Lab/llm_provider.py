...
def get_raw_credentials():
    """
    For the rare case where you need to make a raw HTTP request yourself
    instead of going through get_chat_model()/get_embeddings_model() (e.g.
    bulk-loading/load-testing scripts hitting a vector DB's REST API
    directly). Returns (api_key, cert_path_or_False) for whichever
    environment is active -- office returns the real MODEL_1_APIKEY value
    and IDFC_CERT_PATH; personal returns OPENAI_API_KEY and False (personal
    endpoints don't need a custom cert).
    Prefer get_chat_model()/get_embeddings_model() over this whenever
    possible -- they already handle SSL/retries/routing for you.
    """
    if CURRENT_ENV == "office":
        return _office_api_key(), _office_cert_path()
    return PERSONAL_API_KEY, False
